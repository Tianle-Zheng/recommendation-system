...
class LONGERTokenMerge(nn.Module):
    """LONGER-style token merging with an inner Transformer.

    Reduces a sequence (B, L, D) to (B, L/k, D) by:
      1. Reshaping into windows of k adjacent tokens.
      2. Running a lightweight Transformer (self-attn + FFN) within each
         window so the k tokens interact before merging.
      3. Aggregating each window into one token via a learned query that
         attends to the k processed tokens.

    Lets the model load longer raw histories (e.g. 1024) and compress them
    to a manageable length (256) before downstream attention / DIN pool,
    instead of truncating long-history users.
    """
    ...
    def __init__(
        ...
        # Inner Transformer: 1 layer of self-attn + FFN (Pre-LN)
        ...
        # Aggregation: a learned query attending to the k processed tokens
    ...
    def forward(
        ...
        """Merge adjacent windows of k tokens into one.
        ...
        Returns:
            (merged_tokens, new_padding_mask) of shapes
            (B, L_new, D) and (B, L_new) where L_new = ceil(L / k).
        """
        B, L, D = x.shape
        ...
        # Reshape per-window: (B, L_new, k, D) -> (B * L_new, k, D)
        x = x.view(B, L_new, self.k, D).reshape(B * L_new, self.k, D)
        mask_w = padding_mask.view(B, L_new, self.k).reshape(B * L_new, self.k)

        # Windows that are fully-padded cause NaNs in attention; mark them
        # so we can zero them out at the end.
        all_pad_window = mask_w.all(dim=-1)  # (B * L_new,)
        if all_pad_window.any():
            # Temporarily un-mask one slot so attention is well-defined; the
            # final tokens for these windows are zeroed below.
            mask_w = mask_w.clone()
            mask_w[all_pad_window, 0] = False

        # --- Inner Transformer (Pre-LN) ---
...
class DINPooler(nn.Module):
    ...
    For each sequence position, scores its relevance to a target item via
    an MLP over [item, seq_pos, item-seq_pos, item*seq_pos], then takes a
    softmax-weighted sum of the sequence tokens. Replaces mean pool when
    enabled.

    Optional learnable position embedding (pos_dim > 0) is concatenated
    into the MLP input so the score can be conditioned on recency. With
    latest-first sequence ordering, position 0 = most recent, position
    L-1 = oldest. Disabled by default (pos_dim=0).
    """
    ...
    def forward(
        ...
        if self.pos_dim > 0:
            ...
            pos_feat = self.pos_emb(positions).unsqueeze(0).expand(B, -1, -1)  # (B, L, pos_dim)
            parts.append(pos_feat)
        interaction = torch.cat(parts, dim=-1)                # (B, L, 4D + pos_dim)
        score = self.score_mlp(interaction).squeeze(-1)       # (B, L)
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    Pool is mean pool by default; if ``use_din_pool=True`` and an
    ``item_query`` is supplied at forward time, DINPooler replaces the
    mean pool with item-aware soft attention.
    """
    ...
    def forward(
        ...
        item_query: Optional[torch.Tensor] = None,
    ) -> list:
        ...
        Args:
            ...
            seq_padding_masks: List of (B, L_i) masks, length S. True
                indicates padding.
            item_query: Optional (B, D) target-item representation. Required
                when ``use_din_pool=True``.
...
class GroupNSTokenizer(nn.Module):
    ...
    def get_raw_fid_embeddings(self, int_feats: torch.Tensor) -> torch.Tensor:
        """Return per-fid embeddings stacked along a new token dim.

        Mirrors the embedding lookup in forward(), but stops before group
        projection. Used by DIN's item_query construction so the aggregator
        attends to all individual fid embeddings instead of the compressed
        group tokens.

        Returns:
            (B, total_num_fids, emb_dim) — total_num_fids = sum(len(g) for g in groups).
        """
        all_embs = []
...
class RankMixerNSTokenizer(nn.Module):
    ...
    def get_raw_fid_embeddings(self, int_feats: torch.Tensor) -> torch.Tensor:
        """Return per-fid embeddings without chunk projection. Used by DIN.

        Returns:
            (B, total_num_fids, emb_dim) — preserves individual fid identity
            so the DIN aggregator attends to every fid separately instead of
            the compressed chunked tokens.
        """
        all_embs = []
        for group in self.groups:
            for fid_idx in group:
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Per-domain learnable merge: reduces each seq from L to ceil(L/merge_size)
        # via an inner Transformer + learned-query aggregation. Disabled when
        # merge_size == 1.
        if merge_size > 1:
        ...
        # For DIN's item_query, we attend over RAW item fid embeddings (not the
        # 2 compressed NS tokens) so the aggregator has K=num_item_fids tokens
        # to weight rather than just 2. Projects emb_dim -> d_model when they
        # differ. Active only when use_din_pool=True.
        if use_din_pool:
            if emb_dim != d_model:
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # DIN item_query: attend over RAW item fid embeddings (one per fid)
        # instead of the 2 compressed NS tokens, so the aggregator has real
        # material to learn from.
        if self.use_din_pool:
            item_raw = self.item_ns_tokenizer.get_raw_fid_embeddings(
                inputs.item_int_feats)  # (B, num_item_fids, emb_dim)
            if hasattr(self, 'item_raw_proj'):
                item_raw = self.item_raw_proj(item_raw)  # (B, num_item_fids, D)
        ...
        # 2.5 Optional LONGER-style token merge per domain

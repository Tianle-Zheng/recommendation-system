...
class RankMixerBlock(nn.Module):
    ...
    def _moe_ffn(self, x: torch.Tensor) -> torch.Tensor:
        """Sparse top-k MoE FFN (eval path; aux loss not used for inference)."""
        B, T, D = x.shape
    ...
    def forward(
        ...
        for i in range(self.num_sequences):
            ...
            if top_k_tokens is not None and self.din_top_k > 0:
                q_tokens = torch.cat([q_tokens, top_k_tokens], dim=1)
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                d_model=d_model,
                num_heads=num_heads,
                num_queries=self.effective_num_queries,
                num_ns=self.num_ns,
                num_sequences=self.num_sequences,
